...
==========================================================
Launches the full navigation stack with selectable algorithm:

  algorithm:=dwa   → NavFn (A*) global planner + DWB local controller
  algorithm:=mppi  → Smac Hybrid A* global planner + MPPI local controller

Also starts:
  • dynamic_obstacles node  — moves two obstacle boxes in the Gazebo world
  • metrics_logger node     — records per-mission performance CSV
...
Stack order:
  ...
  → dynamic_obstacles + metrics_logger
  → rviz2
"""
import os
...
def generate_launch_description():
    ...
    nav2_bt = get_package_share_directory('nav2_bt_navigator')
    ...
    # Pre-compute BT XML paths so they are portable across machines
    nav_to_pose_bt_xml = os.path.join(
        nav2_bt, 'behavior_trees',
        'navigate_to_pose_w_replanning_and_recovery.xml')
    nav_through_poses_bt_xml = os.path.join(
        nav2_bt, 'behavior_trees',
        'navigate_through_poses_w_replanning_and_recovery.xml')

    # ── Algorithm-specific param files ───────────────────────────────────────
    params_dwa  = os.path.join(pkg, 'config', 'nav2_params_dwa.yaml')
    params_mppi = os.path.join(pkg, 'config', 'nav2_params_mppi.yaml')

    # ── Launch arguments ─────────────────────────────────────────────────────
    algorithm_arg    = DeclareLaunchArgument(
        'algorithm', default_value='dwa',
        description='Navigation algorithm: dwa | mppi')
    ...
    # Select params file at launch time based on algorithm argument
    params = PythonExpression([
        '"', params_mppi, '" if "', algorithm, '" == "mppi" else "', params_dwa, '"'
    ])

    # ── Gazebo ───────────────────────────────────────────────────────────────
    # libgazebo_ros_state.so enables /gazebo/set_entity_state service
    ...
    bt_navigator = Node(
        ...
        parameters=[params, bt_navigator_params],
    )
    ...
    # Started 12 s after launch to give Gazebo time to fully load
    dynamic_obstacles = TimerAction(
        period=12.0,
        actions=[Node(
    ...
    # ── Nav2 starts after Gazebo has booted ──────────────────────────────────
    ...
    info_start = LogInfo(msg=(
        ...
        '║  algorithm:=dwa  → NavFn A* + DWB   (default)               ║\n'
        '║  algorithm:=mppi → Smac Hybrid A* + MPPI                     ║\n'
        '║                                                              ║\n'
        '║  Run mission:                                                ║\n'
        '║  ros2 run delivery_robot delivery_mission room1 room2        ║\n'
        '║  ros2 run delivery_robot delivery_mission --algorithm mppi \\ ║\n'
        '║                                            room1 room2       ║\n'
        '╚══════════════════════════════════════════════════════════════╝'
    ))

    return LaunchDescription([
        algorithm_arg, map_arg, use_rviz_arg, use_sim_time_arg,
        x_arg, y_arg, z_arg,
        info_start,
        gazebo,
        robot_state_pub,
